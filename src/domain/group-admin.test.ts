...
test("friend candidates retain every MID independently of profile resolution", async () => {
  ...
  const contacts = new ContactsDomain({
    base: {
      relation: {
        getUserFriendIds: async (args: unknown) => {
...
test("invitation cancellation uses the existing nested Talk request and reqSeq", async () => {
  ...
  let sequence = 7;
  const chat = new ChatDomain({
    base: {
      getReqseq: async () => sequence++,
      talk: {
        deleteOtherFromChat: async (args: unknown) => {
          expect(args).toEqual({ request: { reqSeq: 8, chatMid, targetUserMids: [mid] } });
          return {};
        },
